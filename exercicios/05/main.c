...
void resolver(int **matriz, int tamMatriz[2], int **solucao, PILHA *p, PILHA *menor_caso, coords destino, int atual_x, int atual_y, int count);
...
bool solucao(int **matriz, int tamMatriz[2], coords inicial, coords destino) {
    ...
    // chama a funcao que resolve recursivamente o problema
    resolver(matriz, tamMatriz, solucao, p, menor_caso, destino, inicial.x, inicial.y, count);

    // verifica se ha uma solucao valida
    if (pilha_tamanho(menor_caso) == 0) {
    ...
    // inverte a pilha
    pilha_inverter(menor_caso);

    // imprime a solucao
    pilha_print(menor_caso);
...
void resolver(int **matriz, int tamMatriz[2], int **solucao, PILHA *p, PILHA *menor_caso, coords destino, int atual_x, int atual_y, int count) {
    ...
    // verifica se a recursao voltou ao seu ponto inicial
    if (pilha_vazia(p) && (count > 0)) {
        return;
    }

    // verifica se a posicao atual e igual a posicao destino
    if ((atual_x == destino.x) && (atual_y == destino.y)) {
        // atribui 0 na matriz
        solucao[atual_x][atual_y] = 0;

        // cria um item com a posicao atual e empilha
        it = item_criar(atual_x, atual_y);
        pilha_empilhar(p, it);

        // verifica se eh a primeira solucao encontrada
        if (pilha_tamanho(menor_caso) != 0) {
            // verifica se a solucao encotrada eh melhor do que a que ja existe
            if (pilha_tamanho(p) < pilha_tamanho(menor_caso)) {
                pilha_copiar(p, menor_caso);
            }
        } else {
            pilha_copiar(p, menor_caso);
        }

        // desempilha e atribui 1 na matriz
        pilha_desempilhar(p);
    ...
    if (dentroMatriz(matriz, tamMatriz, atual_x, atual_y) == true) {
        ...
        resolver(matriz, tamMatriz, solucao, p, menor_caso, destino, atual_x, atual_y - 1, ++count);

        // verifica se a posicao de cima pode resolver o problema
        resolver(matriz, tamMatriz, solucao, p, menor_caso, destino, atual_x - 1, atual_y, ++count);
        ...
        // atribui 1 a matriz solucao, novamente
        solucao[atual_x][atual_y] = 1;
